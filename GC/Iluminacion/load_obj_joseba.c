...
printf("2 pasada\n");
    ...
    for (int i = 0; i < object_ptr->num_faces; i++){
    	...
    	printf("Vector Normal: (%.4f,%.4f,%.4f)\n",act->normal.x,act->normal.y,act->normal.z);
    ...
    for(int i = 0; i < object_ptr->num_vertices; i++){
    	...
    	GLfloat length = sqrt(pow(act->normal.x,2) + pow(act->normal.y,2) + pow(act->normal.z,2));
    	act->normal.x = act->normal.x / length;
    	act->normal.y = act->normal.y / length;
    	act->normal.z = act->normal.z / length;
    	printf("Vector Normal del Vertice %d: (%.4f,%.4f,%.4f)\n",i,act->normal.x,act->normal.y,act->normal.z);
    }
    ...
    /* Object material initialize*/   
    object_ptr->material = 0;
