...
def actualizar_formula(num_variables,lit, value, clauses,assignment):
    f = deepcopy(clauses)
    assignment[abs(lit)] = value
    if assignment[abs(lit)] == 1:
        f = [clause for clause in f if lit not in clause]
        for clause in f:
                while -lit in clause:
                    clause.remove(-lit)
    else:
...
def solve_SAT_aux(num_variables,clauses,assignment):
    ...
    if [] in clauses:
        ...
        clauses_copy, assignment_copy = actualizar_formula(num_variables,lit,0,clauses,assignment)
        result = solve_SAT_aux(num_variables,clauses_copy,assignment_copy)
        if result != "UNSATISFIABLE":
            return result
        else:
            clauses, assignment = actualizar_formula(num_variables,lit,1,clauses,assignment)
