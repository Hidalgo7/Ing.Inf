...
def has_sum(value, collection):
  #TODO
    if value == 0:
      ...
      # i = 0
      # sum = False
      # while i < len(collection) and not sum:
      #     sum = has_sum(value-collection[i],collection[:i] + collection[i+1:])
      #     i += 1
...
def subset(value, collection):
    #TODO
    return subset_aux(value,collection,[])

def subset_aux(value,collection,subset):
    ...
    if value == 0:
        ...
        return subset_aux(value,collection[1:],subset)
    else:
        sub = subset_aux(value-collection[0],collection[1:],subset + [collection[0]])
        if None in sub:
            sub = subset_aux(value,collection[1:],subset)
