...
def depthFirstSearch(problem):
    ...
    for successor in problem.getSuccessors(problem.getStartState()):
        stack.push((problem.getStartState(),successor))
    reached = False
    
    while stack and not reached:
        next = stack.pop()
        print("Next 1: " + str(next))
        while next[1][0] in checked and not stack.isEmpty():
            next = stack.pop()
        while path[-1] != next[0]:
            path.pop()
